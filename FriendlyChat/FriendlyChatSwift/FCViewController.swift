...
//        signedInStatus(isSignedIn: true)
    ...
    func configureAuth() {
        ...
        NSLog("창남 - print provider \(provider)")
        _authHandle = FIRAuth.auth()?.addStateDidChangeListener { (auth: FIRAuth, user: FIRUser?) in
            NSLog("창남 - authHandle 안 \(auth)")
            // 기존 테이블 refresh
            self.messages.removeAll(keepingCapacity: false)
            self.messagesTable.reloadData()
            
            // current user가 있는지 체크
    ...
    func fetchConfig() {
        ...
        remoteConfig.fetch(withExpirationDuration: expirationDuration, completionHandler: { (status, error) in
            if status == .success {
                ...
                let friendlyMsgLength = self.remoteConfig["friendly_remote_config"]
    ...
    func sendMessage(data: [String:String]) {
        ...
        var dataAddUsername = data
        dataAddUsername[Constants.MessageFields.name] = displayName
        
        ref.child("messages").childByAutoId().setValue(dataAddUsername)
    }
